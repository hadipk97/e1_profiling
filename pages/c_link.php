    ...
    <div class="">
      <div class="page-title">
        <div class="title_left">
          <h3>Padanan Data
            <!--<blink><font color="Red">R&D Stage</font></blink>-->
          </h3>
        </div>
      </div>
          ...
          <div class="col-md-12 col-sm-12 col-xs-12">
            <center>
              <div class="x_panel">
                <div class="x_title">
                  <div class="box-body">
                    <form autocomplete="off" action="c_link.php" method="get>
                          <div class=" row">
                      <div class="col-md-3"></div>
                      <div class="col-md-6" align="Center">
                        <?php for ($i = 0; $i < $f; $i++) { ?>
                          <div id="A<?= $i ?>" <?= ($i == "0" || !empty($_GET['individu' . $i])) ? "" : "class=\"collapse\"" ?>>
                            <div class="autocomplete<?= $i ?>" style="width:300px;">
                              <input id="myauto_complete<?= $i ?>" type="text" name="individu<?= $i ?>" placeholder="Nama <?= ($i == "0") ? "Target" : "Individu" ?> " class="form-control" value="<?php echo (!empty($_GET['individu' . $i])) ? $_GET['individu' . $i] : ""; ?>" <?= ($i == "0") ? "required" : "" ?>>
                            </div>
                            <?php if ($i == ($f - 1)) {
                              echo "&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;";
                            } else { ?><button type="button" class="btn-sm btn btn-default" data-toggle="collapse" data-target="#A<?= $i + 1 ?>"><span>+/-</span></button><?php } ?>
                          </div>
      ...
      if (!empty($_GET['individu0'])) {

        $invidu_firstname = $_GET['individu0'];
      ...
      <div id="ExpendDetail" value="0" onclick="clickDetail();">
        <center>
          <h3>Expend Detail</h3>
        </center>
      </div>
      <script>
        function clickDetail() {
          var togglebtn = document.getElementById("ExpendDetail");
          if (togglebtn.value == null || togglebtn.value == 0) {
            document.getElementById("ExpendDetail").value = 1;
            document.getElementById("detail").classList.remove("hidden");
          } else {
            document.getElementById("ExpendDetail").value = 0;
            ...
            if (!empty($_GET['individu' . $i])) { ?>
              ...
              <?php } else { ?>
                <div class="col-md-<?= $tb ?>">
                  ...
                  ?>
                  <center>
                    <font style="font-size: 50px;color: #<?= $color ?>"><?= $round ?>%</font>
                  </center>
                </div>
              <?php } ?>
          <?php }
