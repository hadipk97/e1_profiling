    ...
    <script>
      $(document).ready(function() {
        $("#add_komoditi").click(function() {
          var list_length = $('#list_komoditi').data('list_length');
          $("#list_komoditi").append($("#list_select_komoditi").clone().css("display", "block").attr('id',
            'list_select_komoditi' + list_length))
          $("#list_select_komoditi" + list_length).find("select").attr('id', 'komoditi_' + list_length);
          $("#list_select_komoditi" + list_length).find("#nokontena_").attr('id', 'nokontena' + list_length);
          $("#list_select_komoditi" + list_length).find("#harga_").attr('id', 'harga_' + list_length);
          $("#list_select_komoditi" + list_length).find("#kuantiti_").attr('id', 'kuantiti_' +
            list_length);
          $("#list_select_komoditi" + list_length).find("#hargatotal_").attr('id', 'hargatotal_' +
            list_length);
          list_length++;
          $('#list_komoditi').data('list_length', list_length);
        });
        $("#clear_komoditi").click(function() {
          $("#list_komoditi").empty();
          $('#list_komoditi').data('list_length', 0);
        });
        $('.trash').click(function() {
          ...
          $.ajax({
            type: 'POST',
            url: 'delete_komoditi.php',
            data: {
              'del_id': del_id
            },
            success: function(data) {
              if (data == 'YES') {
                tr.fadeOut(1, function() { // **add this
                  $(this).remove();
                });
              } else {
          ...
          setTimeout(function() {
            var sumOfAllHargatotalFromDb = 0;
            $('.hargatotalfromdb').each(function() {
              var tdText = $(this).text();
              sumOfAllHargatotalFromDb += parseFloat(tdText);
            });
            var values = $("input[name='hargatotal[]']")
              .map(function() {
                return $(this).val();
              }).get();
            var totalharga_new_komoditi = 0;
            for (var i = 0; i < values.length; i++) {
              totalharga_new_komoditi += values[i] << 0;
            }
            totalAll = totalharga_new_komoditi + sumOfAllHargatotalFromDb;
            $('#total').val('RM ' + totalAll);
          }, 1000)
        });
        $('.trashfakta').click(function() {
          var tr = $(this).closest('tr'); // **add this
          var padam_id = $(this).attr('id');
          console.log(padam_id);
          $.ajax({
            type: 'POST',
            url: 'delete_fakta.php',
            data: {
              'padam_id': padam_id
            },
            success: function(data) {
